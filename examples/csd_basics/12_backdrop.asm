; 12_backdrop.asm - apply DWM system backdrop policy.
;
; This keeps 11 caption fades and keeps opaque GDI paint coverage deterministic
; while cycling DWMWA_SYSTEMBACKDROP_TYPE on capable Windows 11 builds.

ADDON_WINDOWS_RESOURCE equ '12_backdrop.res'
...
macro csd_data
	app_name	du 'CSD 12 - backdrop policy',0
	class_name	du 'Fasm2CsdBackdrop',0
	title_text	du '12  DWM system backdrop',0
	body_text	du 'This version keeps caption fades, the Search child HWND, snap hover, and the real system menu while applying DWMWA_SYSTEMBACKDROP_TYPE.',13,10,13,10
			du 'The Settings glyph cycles None, Mica, Acrylic, and Mica Alt. The normal GDI path still fills the client and caption so resizing and dragging never leave stale pixels.',0
	status_format	du 'Backdrop req %s rd %s | set %08X get %08X | build %u | %s | edit %p | snap %s',0
...
proc UpdateThemeStatus uses rbx rsi rdi r12
  ...
  status_snap_ready:
	...
	invoke	wsprintfW,addr status_buffer,status_format,\
		...
		dword [backdrop_policy.osBuild],rdi,[hSearchEdit],rbx
