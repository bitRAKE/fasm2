; 07_caption_state.asm - add client-area caption hover/pressed state.
;
; This keeps 06 DPI + DWM theme behavior and starts writing the mutable state
; column with ordinary client mouse messages and WM_ACTIVATE.

ADDON_WINDOWS_RESOURCE equ '07_caption_state.res'
...
macro csd_data
	app_name	du 'CSD 07 - caption state',0
	class_name	du 'Fasm2CsdCaptionState',0
	title_text	du '07  Dynamic caption state',0
	body_text	du 'This version keeps 06 DPI and DWM theme behavior, then mutates the state column from ordinary client-area mouse messages.',13,10,13,10
			du 'WM_MOUSEMOVE arms TrackMouseEvent(TME_LEAVE), WM_LBUTTONDOWN/UP owns pressed state, and WM_ACTIVATE maps inactive windows to an inactive caption-button state.',0
