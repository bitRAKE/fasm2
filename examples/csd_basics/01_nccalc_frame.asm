; 01_nccalc_frame.asm - first CSD step: reclaim the frame with WM_NCCALCSIZE.
;
; The program keeps WS_OVERLAPPEDWINDOW but claims the full rectangle offered
; by WM_NCCALCSIZE as client area. It deliberately does not implement
; WM_NCHITTEST yet, so the next example can restore native drag and resize
; behavior as a separate step.
...
CSD_TITLE_BG		= 00D79B31h	; RGB(49,155,215)
...
macro csd_data
	app_name	du 'CSD 01 - WM_NCCALCSIZE',0
	class_name	du 'Fasm2CsdNccalcFrame',0
	title_text	du '01  WM_NCCALCSIZE reclaims the frame',0
	body_text	du 'This window is still created as WS_OVERLAPPEDWINDOW, but the window procedure handles WM_NCCALCSIZE and returns 0 instead of forwarding.',13,10,13,10
			du 'That makes the proposed window rectangle become the client rectangle, so the application owns every visible pixel, including the former title bar and sizing border.',0
	footer_text	du 'ESC closes. Drag and resize are intentionally absent until the WM_NCHITTEST example.',0
