; 14_responsive_caption.asm - overflow and collapse in a tabbed caption.
;
; This keeps 13's tab strip and adds the width-management policy real
; client-side captions need once app-owned controls compete with tabs.

ADDON_WINDOWS_RESOURCE equ '14_responsive_caption.res'
...
macro csd_data
	app_name	du 'CSD 14 - responsive caption',0
	class_name	du 'Fasm2CsdResponsiveCaption',0
	title_text	du '14  Responsive caption overflow',0
	body_text	du 'This version keeps the tab strip, Search child HWND, backdrop policy, snap hover, and real system menu while adding priority collapse.',13,10,13,10
			du 'Narrow the window: Search hides first, Settings and Add move into the app overflow menu, tabs shrink then clip, and WM_GETMINMAXINFO stops impossible sizes.',0
	status_format	du 'Tabs %u/%u active %u | hidden %u | overflow %u | Backdrop %s | build %u | %s | edit HWND %p | snap %s',0
...
proc ShowOverflowMenu uses rbx, hwnd,lparam_value
  ...
  overflow_skip_settings:
	...
	invoke	TrackPopupMenu,rbx,TPM_RETURNCMD or TPM_RIGHTBUTTON or TPM_NONOTIFY,\
		dword [pt.x],dword [pt.y],0,[hwnd],0
	mov	[command],eax
