; 05_dpi_frame.asm - make CSD caption geometry DPI-dependent.
;
; This keeps the 04 descriptor/geometry/state split and makes the geometry
; column a function of the active DPI. WM_DPICHANGED consumes USER's suggested
; window rectangle; color and input state remain intentionally flat.

ADDON_WINDOWS_RESOURCE equ '05_dpi_frame.res'
...
macro csd_data
	app_name	du 'CSD 05 - DPI frame',0
	class_name	du 'Fasm2CsdDpiFrame',0
	title_text	du '05  DPI-scaled caption geometry',0
	body_text	du 'This version keeps the 04 caption-control table split and makes geometry a pure function of DPI and the client rect.',13,10,13,10
			du 'A PMv2 manifest opts in to per-monitor DPI. WM_DPICHANGED applies USER''s suggested window rectangle, and caption widths stay in DIPs until CsdCaptionLayout writes physical RECTs.',0
	status_idle	du 'Move between monitors or change scale: caption controls relayout from DIPs, not hardcoded pixels.',0
...
proc DestroyPaintObjects
	iterate handle, hCaptionFont,hTextFont,hTitleBrush,hButtonBrush,hCloseBrush,hEdgeBrush
		mov	rcx,[handle]
