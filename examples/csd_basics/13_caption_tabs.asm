; 13_caption_tabs.asm - dynamic owner-drawn tabs in the caption.
;
; This keeps 12 backdrop policy and moves the reclaimed caption toward its
; primary use case: a tab strip sharing space with window controls.

ADDON_WINDOWS_RESOURCE equ '13_caption_tabs.res'
...
CSD_CAPTION_SETTINGS_INDEX = 3
CSD_CAPTION_CLOSE_INDEX	= 4
CSD_CAPTION_MAX_INDEX	= 5
CSD_CAPTION_MIN_INDEX	= 6
CSD_TAB_PART_FIXED	= 4
CSD_UIA_TAB_SLOT_ROOT	= -1
CSD_UIA_FIXED_LEADING_COUNT = 3
CSD_UIA_FIXED_TRAILING_COUNT = 3
CSD_UIA_FIXED_SLOT_COUNT = CSD_UIA_FIXED_LEADING_COUNT + CSD_UIA_FIXED_TRAILING_COUNT
CSD_UIA_TAB_SLOT_BASE	= CSD_UIA_FIXED_LEADING_COUNT
CSD_UIA_TAB_SLOT_COUNT	= CSD_TAB_CAPACITY * 2
CSD_UIA_FIXED_TRAILING_SLOT_BASE = CSD_UIA_TAB_SLOT_BASE + CSD_UIA_TAB_SLOT_COUNT
CSD_UIA_SLOT_COUNT	= CSD_UIA_FIXED_TRAILING_SLOT_BASE + CSD_UIA_FIXED_TRAILING_COUNT
...
macro csd_data
	app_name	du 'CSD 13 - caption tabs',0
	class_name	du 'Fasm2CsdCaptionTabs',0
	title_text	du '13  Tabbed caption',0
	body_text	du 'This version keeps backdrop policy, caption fades, the Search child HWND, snap hover, and the real system menu while adding dynamic owner-drawn tabs.',13,10,13,10
			du 'The Add glyph appends a tab, tab bodies select, tab close glyphs compact the parallel item/geometry/state rows, and gaps around tabs remain HTCAPTION drag space.',0
	status_format	du 'Tabs %u/%u active %u | Backdrop %s | build %u | %s | edit HWND %p | snap %s',0
	...
	uia_name_root	du 'CSD 13 - caption tabs',0
	uia_name_close_tab du 'Close tab',0
	uia_name_system du 'System menu',0
	uia_name_newtab du 'New tab',0
	...
	uia_id_root	du 'tabs.root',0
	uia_id_system	du 'caption.system',0
	uia_id_newtab	du 'caption.newtab',0
	...
	uia_fixed_indices dd CSD_CAPTION_SYSTEM_INDEX,CSD_CAPTION_NEWTAB_INDEX,\
			CSD_CAPTION_SETTINGS_INDEX,CSD_CAPTION_MIN_INDEX,\
			CSD_CAPTION_MAX_INDEX,CSD_CAPTION_CLOSE_INDEX
...
macro csd_bss
	...
	uia_children	rb sizeof.CSD_UIA_PROVIDER * CSD_UIA_SLOT_COUNT
...
proc IsTabKeyboardTargetVisible index,part
	cmp	edx,CSD_TAB_PART_FIXED
	jne	tab_key_target_tab
	cmp	ecx,0
	jl	tab_key_target_no
	cmp	ecx,CSD_CAPTION_CONTROL_COUNT
	jae	tab_key_target_no
	imul	eax,ecx,sizeof.CSD_CAPTION_DESCRIPTOR
	test	[caption_descriptors+rax+CSD_CAPTION_DESCRIPTOR.flags],CSD_CAPTION_CHILD
	jnz	tab_key_target_no
	imul	eax,ecx,sizeof.CSD_CAPTION_GEOMETRY
	lea	r10,[caption_geometry+rax+CSD_CAPTION_GEOMETRY.rect]
	mov	eax,[r10+RECT.right]
	cmp	eax,[r10+RECT.left]
	jle	tab_key_target_no
	mov	eax,[r10+RECT.bottom]
	cmp	eax,[r10+RECT.top]
	jle	tab_key_target_no
	mov	eax,1
	ret
  ...
  tab_key_target_check_visible:
	imul	eax,ecx,sizeof.CSD_TAB_ITEM
...
proc MoveTabKeyboardFocus hwnd,delta
  ...
  move_tab_keyboard_active:
	mov	dword [total],CSD_UIA_SLOT_COUNT
	mov	dword [attempts],CSD_UIA_SLOT_COUNT
...
proc InvokeTabKeyboardFocus hwnd
	...
	cmp	dword [tab_keyboard.part],CSD_TAB_PART_FIXED
	je	invoke_tab_keyboard_fixed
  ...
  invoke_tab_keyboard_fixed:
	fastcall InvokeCaptionIndex,[hwnd],dword [tab_keyboard.index]
	ret
...
proc HandleTabKeyDown hwnd,wparam,lparam
  ...
  tab_key_left:
	cmp	dword [tab_keyboard.active],0
	je	tab_key_no
	fastcall MoveTabKeyboardFocus,[hwnd],-1
	ret
...
proc UiaSlotToIndexPart slot
	...
	cmp	ecx,CSD_UIA_FIXED_LEADING_COUNT
	jb	uia_slot_to_part_fixed_leading
	cmp	ecx,CSD_UIA_FIXED_TRAILING_SLOT_BASE
	jae	uia_slot_to_part_fixed_trailing
	sub	ecx,CSD_UIA_TAB_SLOT_BASE
	cmp	ecx,CSD_UIA_TAB_SLOT_COUNT
	jae	uia_slot_to_part_no
	mov	eax,ecx
	shr	eax,1
  ...
  uia_slot_to_part_fixed_leading:
	mov	eax,[uia_fixed_indices+rcx*4]
	mov	edx,CSD_TAB_PART_FIXED
	ret

  uia_slot_to_part_fixed_trailing:
	cmp	ecx,CSD_UIA_SLOT_COUNT
	jae	uia_slot_to_part_no
	sub	ecx,CSD_UIA_FIXED_TRAILING_SLOT_BASE
	add	ecx,CSD_UIA_FIXED_LEADING_COUNT
	mov	eax,[uia_fixed_indices+rcx*4]
	mov	edx,CSD_TAB_PART_FIXED
	ret
...
proc UiaSlotFromIndexPart index,part
  ...
  uia_slot_from_fixed_loop:
	cmp	r10d,CSD_UIA_FIXED_SLOT_COUNT
	jae	uia_slot_from_part_no
	cmp	ecx,[uia_fixed_indices+r10*4]
	je	uia_slot_from_fixed_found
	inc	r10d
	jmp	uia_slot_from_fixed_loop

  uia_slot_from_fixed_found:
	mov	eax,r10d
	cmp	eax,CSD_UIA_FIXED_LEADING_COUNT
	jb	uia_slot_from_fixed_done
	sub	eax,CSD_UIA_FIXED_LEADING_COUNT
	add	eax,CSD_UIA_FIXED_TRAILING_SLOT_BASE

  uia_slot_from_fixed_done:
	ret
...
proc UiaFirstVisibleProvider
  ...
  uia_first_visible_loop:
	cmp	dword [slot],CSD_UIA_SLOT_COUNT
...
proc UiaLastVisibleProvider
	...
	mov	dword [slot],CSD_UIA_SLOT_COUNT - 1
...
proc UiaTabNameForProvider providerp
  ...
  uia_name_fixed_ready:
	cmp	eax,CSD_CAPTION_SYSTEM_INDEX
	je	uia_name_system_ready
	cmp	eax,CSD_CAPTION_NEWTAB_INDEX
	je	uia_name_newtab_ready
	cmp	eax,CSD_CAPTION_SETTINGS_INDEX
	je	uia_name_settings_ready
	cmp	eax,CSD_CAPTION_MIN_INDEX
	je	uia_name_minimize_ready
	cmp	eax,CSD_CAPTION_MAX_INDEX
	je	uia_name_maximize_ready
	cmp	eax,CSD_CAPTION_CLOSE_INDEX
	je	uia_name_close_caption_ready
	jmp	uia_name_none

  uia_name_system_ready:
	mov	rax,uia_name_system
	ret

  uia_name_newtab_ready:
	mov	rax,uia_name_newtab
	ret

  uia_name_settings_ready:
	mov	rax,uia_name_settings
	ret

  uia_name_minimize_ready:
	mov	rax,uia_name_minimize
	ret

  uia_name_maximize_ready:
	invoke	IsZoomed,[hMain]
	test	eax,eax
	jz	uia_name_maximize_normal
	mov	rax,uia_name_restore
	ret

  uia_name_maximize_normal:
	mov	rax,uia_name_maximize
	ret

  uia_name_close_caption_ready:
...
proc UiaTabAutomationIdForProvider providerp
  ...
  uia_id_fixed_ready:
	cmp	eax,CSD_CAPTION_SYSTEM_INDEX
	je	uia_id_system_ready
	cmp	eax,CSD_CAPTION_NEWTAB_INDEX
	je	uia_id_newtab_ready
	cmp	eax,CSD_CAPTION_SETTINGS_INDEX
	je	uia_id_settings_ready
	cmp	eax,CSD_CAPTION_MIN_INDEX
	je	uia_id_minimize_ready
	cmp	eax,CSD_CAPTION_MAX_INDEX
	je	uia_id_maximize_ready
	cmp	eax,CSD_CAPTION_CLOSE_INDEX
	je	uia_id_close_caption_ready
	jmp	uia_id_none
  ...
  uia_id_newtab_ready:
	mov	rax,uia_id_newtab
	ret

  uia_id_settings_ready:
	mov	rax,uia_id_settings
	ret

  uia_id_minimize_ready:
	mov	rax,uia_id_minimize
	ret

  uia_id_maximize_ready:
	mov	rax,uia_id_maximize
	ret

  uia_id_close_caption_ready:
...
proc UiaRoot_ElementProviderFromPoint uses rbx, thisp
	...
	mov	[tab_index],eax
	fastcall UiaSlotFromIndexPart,dword [tab_index],CSD_TAB_PART_FIXED
	jmp	root_point_check_slot
...
proc UiaFragment_BoundingRectangle uses rbx rsi rdi, thisp,pRetVal
  ...
  fragment_bounds_fixed:
	imul	eax,sizeof.CSD_CAPTION_GEOMETRY
	lea	rsi,[caption_geometry+eax+CSD_CAPTION_GEOMETRY.rect]
	jmp	fragment_bounds_copy
...
proc UpdateThemeStatus uses rbx rsi rdi
	mov	rbx,snap_off
  ...
  status_snap_ready:
	...
	invoke	wsprintfW,addr status_buffer,status_format,\
		...
		dword [tabstrip.activeIndex],rsi,dword [backdrop_policy.osBuild],\
		rdi,[hSearchEdit],rbx
...
proc InvokeCaptionIndex uses rbx, hwnd,index
	...
	mov	[hwnd],rcx
	cmp	edx,CSD_CAPTION_NO_INDEX
	je	caption_invoke_no
	cmp	edx,CSD_CAPTION_CONTROL_COUNT
	jae	caption_invoke_no
	imul	edx,sizeof.CSD_CAPTION_DESCRIPTOR
	lea	rbx,[caption_descriptors+rdx]
  ...
  caption_invoke_not_syscmd:
	...
	je	caption_invoke_newtab
	cmp	edx,ID_CAPTION_SETTINGS
	je	caption_invoke_settings
	jmp	caption_invoke_no

  caption_invoke_newtab:
	fastcall AddDemoTab
	test	eax,eax
	jz	caption_invoke_no
	fastcall LayoutCaptionControls
...
proc WindowProc hwnd,wmsg,wparam,lparam
  ...
  wnd_syskeydown:
	fastcall HandleTabSysKeyDown,[hwnd],[wparam],[lparam]
	test	eax,eax
	jz	wnd_default
	xor	eax,eax
	ret
  ...
  wnd_keydown:
	fastcall HandleTabKeyDown,[hwnd],[wparam],[lparam]
	test	eax,eax
