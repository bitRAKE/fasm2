; 06_dwm_theme.asm - apply DWM theme attributes and live colors.
;
; This keeps the 05 DPI geometry path and adds the other half of DWM work:
; immersive dark mode, caption/border/text colors, corner preference, and live
; refresh on WM_SETTINGCHANGE / WM_DWMCOLORIZATIONCOLORCHANGED.

ADDON_WINDOWS_RESOURCE equ '06_dwm_theme.res'
...
macro csd_data
	app_name	du 'CSD 06 - DWM theme',0
	class_name	du 'Fasm2CsdDwmTheme',0
	title_text	du '06  DWM theme attributes',0
	body_text	du 'This version keeps 05 DPI-scaled geometry and adds live theme inputs.',13,10,13,10
			du 'DwmSetWindowAttribute applies immersive dark mode, caption color, border color, text color, and corner preference. WM_SETTINGCHANGE and WM_DWMCOLORIZATIONCOLORCHANGED rebuild the theme row and repaint.',0
	status_format	du 'Theme gen %u | %s | accent COLORREF 0%06Xh',0
...
proc DestroyThemeBrushes
	iterate handle, hBodyBrush,hTitleBrush,hButtonBrush,hCloseBrush,hEdgeBrush
		mov	rcx,[handle]
...
proc UpdateThemeStatus uses rbx
	mov	rbx,theme_light
	cmp	dword [current_theme.darkMode],0
	je	status_ready
	mov	rbx,theme_dark
  status_ready:
	invoke	wsprintfW,addr status_buffer,status_format,\
		dword [current_theme.generation],rbx,\
		dword [current_theme.accentColor]
...
proc DrawCaptionControls uses rbx rsi rdi r12 r13 r14, hdc
  ...
  draw_glyph_ready:
	fastcall FontIcon_DrawGlyph,rbx,r13d,r12,[hCaptionFont],\
		dword [current_theme.titleTextColor]
