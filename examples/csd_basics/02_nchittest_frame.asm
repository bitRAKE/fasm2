; 02_nchittest_frame.asm - restore native move/resize with WM_NCHITTEST.
;
; WM_NCCALCSIZE makes the full HWND client-owned. WM_NCHITTEST maps selected
; pixels back to native frame roles so USER can run the normal move, resize,
; snap, and cursor behavior.
...
macro csd_data
	app_name	du 'CSD 02 - WM_NCHITTEST',0
	class_name	du 'Fasm2CsdNchittestFrame',0
	title_text	du '02  WM_NCHITTEST restores native input',0
	body_text	du 'The non-client frame is still reclaimed with WM_NCCALCSIZE. This version also handles WM_NCHITTEST and returns native HT codes.',13,10,13,10
			du 'The top band returns HTCAPTION, so dragging and snap layouts are handled by Windows. The outer strip returns the resize edge and corner codes.',0
	footer_text	du 'ESC closes. Drag the top band or resize from any edge.',0
