; 04_caption_controls.asm - use the reclaimed caption area for controls.
;
; The caption surface is driven by immutable glyph descriptors, separate
; runtime geometry, and reserved state rows. The remaining strip between left
; and right controls is the only drag region.
...
CSD_ICON_PX		= 18
CSD_DRAG_PAD_DIP	= CSD_CONTENT_PAD / 2

CSD_TITLE_BG		= 003E8B52h	; RGB(82,139,62)
CSD_BUTTON_BG		= 005A9B64h	; RGB(100,155,90)
CSD_CLOSE_BG		= 004242D7h	; RGB(215,66,66)
CSD_EDGE_COLOR		= 00545759h	; RGB(89,87,84)
CSD_TITLE_TEXT		= 00FFFFFFh
CSD_BODY_TEXT		= 00242120h
CSD_MUTED_TEXT		= 007A6D61h
CSD_STATUS_TEXT		= 00A45F17h
...
macro csd_data
	app_name	du 'CSD 04 - caption controls',0
	class_name	du 'Fasm2CsdCaptionControls',0
	title_text	du '04  Caption area controls',0
	body_text	du 'The reclaimed caption now hosts a data-driven array of controls rendered with examples/font_icons/font_icons.inc.',13,10,13,10
			du 'The menu glyph opens the real system menu. Search and Settings are app-owned client buttons. The right-side controls post WM_SYSCOMMAND without letting USER draw legacy caption buttons over this surface.',0
	status_idle	du 'Click Search or Settings in the caption. Drag only the open strip between controls.',0
...
proc RebuildCaptionFont
  ...
  rebuild_create:
	fastcall FontIcon_CreateFontForWindow,[hMain],CSD_ICON_PX,'Segoe Fluent Icons'
...
proc CreatePaintObjects
	iterate <handle,color>,\
		hTitleBrush,	CSD_TITLE_BG,\
		hButtonBrush,	CSD_BUTTON_BG,\
		hCloseBrush,	CSD_CLOSE_BG,\
		hEdgeBrush,	CSD_EDGE_COLOR
...
proc DestroyPaintObjects
	iterate handle, hCaptionFont,hTitleBrush,hButtonBrush,hCloseBrush,hEdgeBrush
		mov	rcx,[handle]
...
proc LayoutCaptionControls
	...
	fastcall CsdCaptionLayout,caption_descriptors,caption_geometry,\
		CSD_CAPTION_CONTROL_COUNT,addr client_rect,CSD_DPI_BASE,\
...
proc DrawCaptionControls uses rbx rsi rdi r12 r13 r14, hdc
	mov	rbx,rcx
  ...
  draw_loop:
	...
	jz	draw_normal
	invoke	FillRect,rbx,r12,[hCloseBrush]
	jmp	draw_glyph

  draw_normal:
	invoke	FillRect,rbx,r12,[hButtonBrush]
  ...
  draw_glyph_ready:
	fastcall FontIcon_DrawGlyph,rbx,r13d,r12,[hCaptionFont],CSD_TITLE_TEXT
...
proc PaintFrame uses rbx, hdc
	...
	sub	eax,CSD_TITLE_HEIGHT_DIP - 8
	mov	[status_rect.top],eax
	mov	eax,[client_rect.bottom]
	sub	eax,CSD_EDGE_DIP + 8
	mov	[status_rect.bottom],eax
	invoke	SetTextColor,rbx,CSD_STATUS_TEXT
...
proc WindowProc hwnd,wmsg,wparam,lparam
  ...
  wnd_lbuttonup:
	fastcall HandleCaptionClientClick,[hwnd],r9
	xor	eax,eax
	ret
...
proc start
	...
	invoke	CreateWindowEx,WS_EX_APPWINDOW,class_name,app_name,\
		WS_OVERLAPPEDWINDOW,\
		CW_USEDEFAULT,CW_USEDEFAULT,920,500,\
		0,0,[hInstance],0
	mov	[hMain],rax
	test	rax,rax
